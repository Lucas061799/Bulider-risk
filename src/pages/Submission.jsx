...
// Very rough placeholder rating: ~$0.45/$1k of completed value for BR, ~$1.20/$1k for vacant.
// Real ratings come from carrier API responses; this is just for the UI mock.
function estimatePremium(carrierId, completedValue, isVacant) {
  ...
  // Carrier-specific multipliers (illustrative)
...
export default function Submission({ formData, projectType, state, onBack, isDark, onToggleDark }) {
  ...
  // Figure out the value being insured
  ...
  // Eligible carriers for this risk
  ...
  const carriers = eligibleIds.map(id => {
    ...
    return {
      ...c,
      premium: estimatePremium(c.id, completedValue, isVacant),
    }
  }).filter(Boolean)

  return (
    <div className="min-h-screen font-montserrat" style={{ background: isDark ? '#131629' : 'white' }}>
      <Confetti />
      ...
      >
        <div className="flex items-center h-full px-5">
          <button onClick={onBack} className="mr-3 p-1.5 rounded-lg" style={{ color: isDark ? '#9CA3AF' : '#6B7280' }}>
      ...
      <div className="max-w-5xl mx-auto px-6 py-8 md:py-12">
        ...
        <div className="text-center mb-8">
          <div className="inline-flex items-center justify-center w-16 h-16 rounded-full mb-4"
            ...
            <svg className="w-8 h-8" fill="none" stroke="#0D8B73" strokeWidth="2" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" d="M5 13l4 4L19 7"/>
            </svg>
          </div>
          <p className="text-xs font-bold tracking-widest uppercase mb-2" style={{
            background: 'linear-gradient(88.09deg, #5C2ED4 0%, #A614C3 100%)',
            WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent',
          }}>
            Application Submitted
          </p>
          <h1 className="text-3xl md:text-4xl font-bold mb-2" style={{ color: isDark ? '#F9FAFB' : '#1F1B47' }}>
            Compare Your Quotes
          </h1>
          <p className="text-sm text-gray-500">
            Submission <span className="font-bold">{SUBMISSION_ID}</span> · {cfg?.label || projectType} · {projectStateAbbr}
          </p>
        </div>
        ...
        {carriers.length === 0 ? (
          ...
          <div className={`grid gap-4 ${carriers.length === 1 ? 'grid-cols-1 max-w-md mx-auto' : carriers.length === 2 ? 'grid-cols-1 md:grid-cols-2' : 'grid-cols-1 md:grid-cols-3'}`}>
            {carriers.map(c => (
              <CarrierCard
                key={c.id}
                carrier={c}
                ...
                onSelect={() => setSelectedCarrier(c.id)}
                isDark={isDark}
              />
            ))}
          </div>
        ...
        {/* Footer info */}
        <div className="mt-10 p-5 rounded-2xl" style={{
